-- ==============================================================================
-- CLAIMSO SAMPLE DATA SCRIPT FOR SUPABASE (FIXED VERSION)
-- ==============================================================================
...
-- Clear existing sample data for your user ID (optional - comment out if you want to keep existing data)
    ...
    FOR i IN 1..1000 LOOP
        ...
        CASE (i % 10)
            ...
            WHEN 6 THEN -- Sports & Fitness (10%)
                ...
                purchase_price := 30 + (i % 500);
                condition := conditions[1 + (i % array_length(conditions, 1))];
                
            WHEN 7 THEN -- Mixed Categories (10%)
                brand := 'Generic Brand';
                current_product_name := 'Sample Product ' || i;
                category := categories[1 + (i % array_length(categories, 1))];
                purchase_price := 10 + (i % 1000);
                condition := conditions[1 + (i % array_length(conditions, 1))];
                
            WHEN 8 THEN -- High Value Items (10%)
                brand := 'Premium Brand';
                current_product_name := 'Premium ' || electronics[1 + (i % array_length(electronics, 1))];
                category := 'Electronics';
                purchase_price := 1000 + (i % 5000);
                condition := conditions[1 + (i % array_length(conditions, 1))];
                
            ELSE -- Low Value Items (10%)
                brand := 'Budget Brand';
                current_product_name := 'Budget ' || clothing_items[1 + (i % array_length(clothing_items, 1))];
                category := 'Clothing';
                purchase_price := 5 + (i % 50);
                condition := conditions[1 + (i % array_length(conditions, 1))];
        END CASE;
        ...
        serial_number := 'SN-' || LPAD(i::TEXT, 8, '0');
        purchase_date := CURRENT_DATE - INTERVAL '1 day' * (i % 365);
        warranty_count := CASE WHEN (i % 3) = 0 THEN 0 ELSE 1 END; -- 33% of products have no warranty
        ...
        IF warranty_count > 0 THEN
            ...
            SELECT 
                ...
                '{"covers": ["Parts", "Labor", "Defects"], "does_not_cover": ["Accidental damage", "Wear and tear"], "key_terms": ["12 months", "Limited warranty"], "claim_requirements": ["Proof of purchase", "Serial number"]}'::jsonb,
                0.85,
                NOW(),
                NOW(),
                NOW()
...
SELECT 
    ...
    END,
    CASE (ROW_NUMBER() OVER (ORDER BY p.id) % 5)
        WHEN 0 THEN 'Local Auto Shop'
        WHEN 1 THEN 'Dealership Service'
        WHEN 2 THEN 'Mobile Mechanic'
        WHEN 3 THEN 'Chain Auto Center'
        ELSE 'Independent Garage'
    END,
    '555-0123',
    CASE (ROW_NUMBER() OVER (ORDER BY p.id) % 5)
        WHEN 0 THEN 150.00
...
AND p.user_id = '5cbd0756-963e-4777-96d7-629edf66e0ca'
LIMIT 200;
...
VALUES 
    (gen_random_uuid(), '5cbd0756-963e-4777-96d7-629edf66e0ca', 'Amazon', 'connected', NOW() - INTERVAL '1 day', NOW(), NOW()),
    (gen_random_uuid(), '5cbd0756-963e-4777-96d7-629edf66e0ca', 'Best Buy', 'connected', NOW() - INTERVAL '2 days', NOW(), NOW()),
    (gen_random_uuid(), '5cbd0756-963e-4777-96d7-629edf66e0ca', 'Walmart', 'connected', NOW() - INTERVAL '3 days', NOW(), NOW()),
    (gen_random_uuid(), '5cbd0756-963e-4777-96d7-629edf66e0ca', 'Target', 'connected', NOW() - INTERVAL '1 week', NOW(), NOW()),
    (gen_random_uuid(), '5cbd0756-963e-4777-96d7-629edf66e0ca', 'Apple Store', 'connected', NOW() - INTERVAL '2 weeks', NOW(), NOW())
...
-- Check the results
SELECT 'Products' as table_name, COUNT(*)::TEXT as count FROM products WHERE user_id = '5cbd0756-963e-4777-96d7-629edf66e0ca'
UNION ALL
SELECT 'Warranties' as table_name, COUNT(*)::TEXT as count FROM warranties WHERE user_id = '5cbd0756-963e-4777-96d7-629edf66e0ca'
UNION ALL
SELECT 'Maintenance Records' as table_name, COUNT(*)::TEXT as count FROM maintenance_records mr 
JOIN products p ON mr.product_id = p.id WHERE p.user_id = '5cbd0756-963e-4777-96d7-629edf66e0ca'
UNION ALL
SELECT 'User Connections' as table_name, COUNT(*)::TEXT as count FROM user_connections WHERE user_id = '5cbd0756-963e-4777-96d7-629edf66e0ca';
